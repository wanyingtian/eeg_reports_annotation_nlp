...
| False-negative consequences | `eeg-review error-review`; governed FN/FP worksheet with pseudonymous case handles and separate source-key lookup | Clinical reviewer, approved protocol, and patient key for clustered selection | Mistral packets contain 194 Zoe and 145 Maria selected label-case rows. MedGemma v1 packets contain 181 Zoe and 90 Maria rows, with every handle resolvable through a separate governed lookup. A paired focal-error audit further localizes the Zoe trade-off to 22 MedGemma-only false positives and 2 recovered Mistral misses. Clinical adjudication remains team work. |
| Stronger LLM comparisons | [`CONTEMPORARY_LLM_COMPARISON_PROTOCOL.md`](CONTEMPORARY_LLM_COMPARISON_PROTOCOL.md); matched-historical Q2 receipt; frozen native-interface plan; fail-closed protected runner; source-hashed author-review finalizer | Patient key only for patient-grouped inference; exact external v5g bundle for reproducing that distinct configuration | Independent Q2 evaluation is complete and preserved. The native-interface configuration was selected once on the fixed first 100 Zoe reports, frozen before evaluation, and completed on the exact 1,395/499 held-out populations; its 90-row author-review claim package is available. External v5g remains additive and separately gated on exact intake. |
...
| BERT input fairness and truncation | [`BERT_INPUT_FAIRNESS_SENSITIVITY_2026-09-07.md`](BERT_INPUT_FAIRNESS_SENSITIVITY_2026-09-07.md); frozen exact-tokenizer audit and same-population rescore across five saved model surfaces | Complete evaluation reports and saved predictions; no model inference | Completed: 1/1,395 Zoe and 3/499 Maria reports exceeded 512 BERT tokens. Two Maria reports had a recognized downstream heading beyond the retained boundary. Excluding all four from every model changed no Core Agreement ranking; BERT changed at most 0.106 Core Agreement and 0.310 F1 percentage points. |
...
| 97.8% explanation traceability | Positive learned-polarity denominator, exact matcher stages, embedding revision and raw matched/unmatched counts | Submitted-history wording or replacement with a separately named reproducible contract | Denominator 2,180 and source artifact are reconciled. Public-script and thesis-prose replays are 2,018/2,180 and 1,911/2,180; neither reproduces 2,132. The historical producer search is exhausted without threshold tuning. A cross-model exact-source/candidate audit now supplies a reproducible forward path. |
...
4. Promote the completed deterministic MedGemma authoring bundle only through
   the hash-bound admission contract and explicit author review.
