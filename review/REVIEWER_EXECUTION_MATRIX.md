...
| Calibration | `eeg-review calibrate`; Brier, log loss, fixed-bin ECE and bin supports with cluster intervals | Per-class probabilities and patient key; not ordinal labels alone | Historical and fresh Zoe/Maria BoW/BERT receipts completed; clustered intervals await the patient key. Historical Mistral outputs remain ordinal-only; opt-in grammar-token probability capture is implemented for a separately frozen development/evaluation run but has not been run or calibrated |
