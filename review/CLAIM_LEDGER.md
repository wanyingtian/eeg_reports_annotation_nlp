...
| Evidence-informed prompt diagnosis | Saved classifications, reference annotations, literal source spans, cross-model observations and explicit review questions | Additive source-span and failure-coverage audit implemented with abstention and whitespace-only diagnostic recovery. This is not the thesis's fuzzy/semantic factuality metric, trained polarity score, causal faithfulness or clinical validation. Keyed records remain governed. |
