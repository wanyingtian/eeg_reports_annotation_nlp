...
| MedGemma category-scope v2.1 | Separately frozen prompt, same 100 development keys, both parent outputs, independent first-20 evidence audit and exact runtime receipts | In progress August 31: the saved three-report technical smoke passed without inspecting reference metrics; checkpointed continuation started. No completed performance result yet. Reuses the native model/classification grammar and adds category-scope checks. Its audit receives no labels and cannot revise classifications. A distinct exploratory rule retains both parents and every category; no automatic protected evaluation follows. See `MEDGEMMA_V21_UPGRADE_PREPARATION.md`. |
