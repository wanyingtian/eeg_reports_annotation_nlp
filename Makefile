...
.PHONY: sync sync-all lint test audit-sample verify ci verify-evaluation-surfaces verify-llm-receipt preload-model smoke-model smoke-classification study-status study-ledger medgemma-readiness medgemma-prepare medgemma-tier-dry-run medgemma-tier-status medgemma-tier-run medgemma-native-authorization-check medgemma-native-prepare medgemma-native-dry-run medgemma-native-launch medgemma-native-finalize medgemma-native-author-bundle evidence-review-build evidence-review-verify evidence-clinical-delivery-build evidence-clinical-delivery-verify evidence-review-compare reference-traceability-run reference-traceability-verify evidence-factorial-analyze evidence-factorial-verify evidence-profile-verify followup-paper-verify governed-run-eclipse
...
lint:
	...
	$(UV_RUN) ruff check scripts/mistral_interface_followup.py scripts/run_fixed_classification_explanations.py scripts/medgemma_prompt_v2.py scripts/audit_medgemma_v2_evidence.py scripts/audit_medgemma_v21.py
	$(UV_RUN) ruff check src/eeg_review tests scripts/smoke_inference_receipt.py scripts/smoke_model.py scripts/smoke_classification.py scripts/preload_model.py scripts/study_job.py scripts/prepare_medgemma_study.py scripts/run_tiered_medgemma_study.py scripts/medgemma_mission_control.py scripts/benchmark_medgemma_runtime.py scripts/finalize_medgemma_result_candidate.py scripts/finalize_medgemma_native_development.py scripts/finalize_medgemma_native_protected_result.py scripts/render_medgemma_native_author_bundle.py scripts/check_medgemma_native_protected_authorization.py scripts/eclipse_governed_run.py src/LLM_pipeline/llm_models.py

test:
	$(UV_RUN) $(TEST_EXTRAS) pytest

audit-sample:
	$(UV_RUN) eeg-review audit \
...
# Use one locked, CPU-only environment for all hosted checks. Heavy model and
# GPU runtimes remain explicit local/governed-hardware operations.
ci:
	$(MAKE) UV_RUN="$(UV) run --locked --extra ci" TEST_EXTRAS= verify
...
reference-traceability-run:
	$(UV_RUN) python scripts/analyze_reference_aligned_traceability.py

reference-traceability-verify:
	$(UV_RUN) python scripts/analyze_reference_aligned_traceability.py --verify-only

evidence-factorial-analyze:
	$(UV_RUN) python scripts/analyze_evidence_schema_factorial.py

evidence-factorial-verify:
	$(UV_RUN) python scripts/analyze_evidence_schema_factorial.py --verify-only

evidence-profile-verify:
	$(UV_RUN) python scripts/verify_evidence_profile_catalog.py

followup-paper-verify:
	$(UV_RUN) python scripts/verify_followup_paper_draft.py
